...
async function validateJobs(cif) {
  console.log("=== Validate Job URLs from API ===\n");
...
async function deleteExpiredJobs(expiredJobs) {
  ...
  console.log(`\nDeleting ${expiredJobs.length} expired jobs from API...`);
...
Examples:
  node scraper/validate-jobs.js 18643343                 - Validate UTILBEN jobs
  node scraper/validate-jobs.js --url "https://www.ejobs.ro/user/locuri-de-munca/test/123"
  ...
  if (mode === "cif" && cif) {
    ...
    if (results.expired.length > 0) {
      ...
      if (shouldDelete) {
        ...
        console.log("\nPass --delete to remove expired jobs from API");
